...
const SKVector3 SKVector3::ms_Up = SKVector3(0.0f, 1.0f, 0.0f);
const SKVector3 SKVector3::ms_Right = SKVector3(1.0f, 0.0f, 0.0f);
const SKVector3 SKVector3::ms_Front = SKVector3(0.0f, 0.0f, 1.0f);
const SKVector3 SKVector3::ms_Zero = SKVector3(0.0f, 0.0f, 0.0f);
const SKVector3 SKVector3::ms_One = SKVector3(1.0f, 1.0f, 1.0f);
/*----------------------------------------------------------------*/
SKVector3::SKVector3(void) 
{
	x=0;y=0; z=0; 
}
/*----------------------------------------------------------------*/
SKVector3::SKVector3(SKREAL _x, SKREAL _y, SKREAL _z)
{
	x=_x; y=_y; z=_z;
}
/*----------------------------------------------------------------*/

FORCEINLINE SKREAL SKVector3::AngleWith(SKVector3 &v)
{
	return (SKREAL)acos( ((*this).Dot(v)) / (this->GetLength()*v.GetLength()) );
}
...
FORCEINLINE void SKVector3::Create(const SKVector3 &v1, const SKVector3 &v2) 
{
	#ifdef SK_SSE
	{
...
/*----------------------------------------------------------------*/
SKVector3 & SKVector3::operator += (const SKVector3 &v)
{
	#ifdef SK_SSE
	{
...
SKVector3 SKVector3::operator + (const SKVector3 &v) const 
{
	#ifdef SK_SSE
	{
		__m128 _v1 = _mm_set_ps(x,y,z,0);
		__m128 _v2 = _mm_set_ps(v.x,v.y,v.z,0);
		__m128 _v3 = _mm_add_ps(_v2,_v1);
		return SKVector3(_v3.m128_f32[3], _v3.m128_f32[2], _v3.m128_f32[1]);
	}
	#else
	{
...
SKVector3 & SKVector3::operator /= (const SKVector3 &v)
{
#ifdef SK_SSE
	{
		...
		__m128 _v2 = _mm_set_ps(v.x, v.y, v.z, 1.0f);
		__m128 _v3 = _mm_div_ps(_v1, _v2);
		*this = SKVector3(_v3.m128_f32[3], _v3.m128_f32[2], _v3.m128_f32[1]);
		return *this;
	}
#else
	{
		*this =  SKVector3(x / v.x, y / v.y, z / v.z);
		return *this;
	}
#endif
}
SKVector3 SKVector3::operator * (const SKVector3 &v) const
{
	...
	#else
	{
		return return SKVector3(x * v.x, y * v.y, z * v.z);
	}
	#endif
}
SKVector3 & SKVector3::operator *= (const SKVector3 &v) 
...
SKVector3 & SKVector3::operator *= (SKREAL f)
{
	#ifdef SK_SSE
	{
		__m128 _v1 = _mm_set_ps(x,y,z,0);
...
SKVector3 SKVector3::operator * (SKREAL f) const 
{
	#ifdef SK_SSE
	{
		__m128 _v1 = _mm_set_ps(x,y,z,0);
...
/*----------------------------------------------------------------*/
SKREAL SKVector3::Dot(const SKVector3 &v)const
{
	#ifdef SK_SSE
	{
		__m128 _v1 = _mm_set_ps(x, y, z, 0);
...
/*----------------------------------------------------------------*/
bool SKVector3::operator ==(const SKVector3 &v)const
{
...
/*----------------------------------------------------------------*/
SKQuat SKVector3::operator * (const SKQuat &q) const 
{
	#ifdef SK_SSE
	{
	...
	#else
	{
		return SKQuat(  q.w*x + q.z*y - q.y*z,
					q.w*y + q.x*z - q.z*x,
					q.w*z + q.y*x - q.x*y,
					-(q.x*x + q.y*y + q.z*z) );
	}
...
/*----------------------------------------------------------------*/
SKVector3 SKVector3::operator * (const SKMatrix3X3 &m) const 
{
	SKVector3 vcResult;
...
/*----------------------------------------------------------------*/
bool SKVector3::IsParallel(const SKVector3 & Vector)const
{
	SKREAL t0,t1;
	bool temp = 0;
	t0 = x * Vector.y;
...
/*----------------------------------------------------------------*/
SKVector3 SKVector3::operator * (const SKMatrix3X3W &m)const
{
	SKVector3 vcResult;
	...
	#else
	{
		...
		SKREAL w = x*m._03 + y*m._13 + z*m._23 + m._33;
...
SKVector3 SKVector3::ReflectDir(const SKVector3 &N)const
{
