...
namespace SKEngine2
{
	...
	public:
		...
		void operator= (const SKBit<T> &Bit)
		{
			m_Member = Bit.m_Member;
		}
	...
	{
	public:
		SKBitArray(const SKBitArray & BitArray)
		{
			SKMAC_DELETEA(m_pBuffer);
			m_uiSize = BitArray.m_uiSize;
			m_pBuffer = SK_NEW unsigned int[m_uiSize];
			SKMemcpy(m_pBuffer, BitArray.m_pBuffer, sizeof(unsigned int)* m_uiSize);
		}
		SKBitArray( unsigned int uiSize = 0)
		{
			m_pBuffer = 0;
			m_uiSize = 0;
			Resize( uiSize );
		...
		void Resize( unsigned int uiSize )
		{
			...
			pNewVector = SK_NEW unsigned int[uiSize];
			

			if( !pNewVector)
				return;
			SKMemset(pNewVector,0,uiSize * sizeof(unsigned int));
			unsigned int uiMin;
			if( uiSize < m_uiSize )
				uiMin = uiSize;
			else
		...
		void operator= (const SKBitArray & BitArray)
		{
			m_uiSize = BitArray.m_uiSize;
			SKMAC_DELETEA(m_pBuffer);
			m_pBuffer = SK_NEW unsigned int[m_uiSize]; 
			SKMemcpy(m_pBuffer,BitArray.m_pBuffer,m_uiSize * sizeof(unsigned int));
		}
		void Set( unsigned int uiIndex, bool bValue )
		{
			int cell = uiIndex / SYSTEM_BIT_WIDTH;
