...
SKMap<KEY,VALUE,MMFun>::~SKMap()
{
	Delete(m_pBuffer,m_uiCurUse);
}

template<class KEY,class VALUE,SKMemManagerFun MMFun>
SKMap<KEY,VALUE,MMFun>::SKMap(unsigned int uiGrowBy)
{
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::SetBufferNum(unsigned int uiBufferNum)
{
	if(uiBufferNum)
	{
		Delete(m_pBuffer,m_uiCurUse);
		...
		{
			SK_NEW (m_pBuffer + i) MapElement<KEY,VALUE>();
		}
	}	

}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::AddBufferNum(unsigned int uiBufferNum)
{
	if(uiBufferNum)
	{
		//Ҫڴ棬Ͱm_uiGrowBy
		...
		if(m_pBuffer && m_uiCurUse)
		{
			...
			{
				SK_NEW (pBuffer + i) MapElement<KEY,VALUE>(m_pBuffer[i]);
			}
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
FORCEINLINE unsigned int SKMap<KEY,VALUE,MMFun>::GetNum()const
{
	return m_uiCurUse;
}
template<class KEY,class VALUE,SKMemManagerFun MMFun>	
FORCEINLINE unsigned int SKMap<KEY,VALUE,MMFun>::GetBufferNum()const
{
	return m_uiBufferNum;
}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
FORCEINLINE MapElement<KEY,VALUE> * SKMap<KEY,VALUE,MMFun>::GetBuffer()const
{
	return m_pBuffer;
}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
FORCEINLINE void SKMap<KEY,VALUE,MMFun>::SetGrowBy(unsigned int uiGrowBy)
{
	m_uiGrowBy = uiGrowBy;
}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
FORCEINLINE unsigned int SKMap<KEY,VALUE,MMFun>::GetSize()const
{
	return sizeof(unsigned int ) * 4 + sizeof(MapElement<KEY,VALUE> *) + sizeof(MapElement<KEY,VALUE>) * m_uiBufferNum;
}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::AddElement(const KEY& Key , const VALUE &Value)
{
	...
	Element.Key = Key;
	Element.Value = Value;
	SK_NEW (m_pBuffer + m_uiCurUse) MapElement<KEY,VALUE>(Element);
	m_uiCurUse++;

}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
template<class KEY1,class VALUE1>
void SKMap<KEY,VALUE,MMFun>::AddElement(const MapElement<KEY1,VALUE1> & Element)
{
...
void SKMap<KEY,VALUE,MMFun>::AddElement(const SKMap<KEY1,VALUE1,MMFun1> & Map,unsigned int uiBegin,unsigned int uiEnd)
{
	if (uiBegin > uiEnd)
	{
		return ;
	...
	else
	{
		...
		{
			SK_NEW (m_pBuffer + i) MapElement<KEY,VALUE>(pBuffer[uiBegin]);
			uiBegin++;
		}
		m_uiCurUse += iAddNum;
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::operator= (const SKMap<KEY,VALUE,MMFun>& Map)
{
	if (m_uiBufferNum >= Map.GetNum())
	{
		...
		for (unsigned int i = 0 ;i < m_uiCurUse ; i++)
		{
			SK_NEW (m_pBuffer + i) MapElement<KEY,VALUE>(pBuffer[i]);
		}
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::Clear()
{
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::Destroy()
{
	Clear();
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
MapElement<KEY,VALUE>& SKMap<KEY,VALUE,MMFun>::operator[] (unsigned int i)const
{
#ifdef _DEBUG
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::Erase(unsigned int i)
{
	Erase(i,i);
}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::Erase(unsigned int uiBegin,unsigned int uiEnd)
{
	if (uiEnd >= m_uiCurUse)
	{
		return;
	...
	for (k = 0 ; k < uiMoveNum ; k++)
	{
		...
		{		
			SK_NEW (m_pBuffer + uiBegin + k) MapElement<KEY,VALUE>(m_pBuffer[uiMoveIndex]);
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
template <class N>
void SKMap<KEY,VALUE,MMFun>::Sort(unsigned int uiBegin,unsigned int uiEnd,N Compare)
{
	if (uiEnd >= m_uiCurUse)
	{
		uiEnd = m_uiCurUse - 1;
	...
	{
		SK_NEW(pBuffer + i) MapElement<KEY, VALUE> ();
	}
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMap<KEY,VALUE,MMFun>::Sort(unsigned int uiBegin,unsigned int uiEnd)
{
	if (uiEnd >= m_uiCurUse)
	{
		uiEnd = m_uiCurUse - 1;
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
unsigned int SKMap<KEY,VALUE,MMFun>::Find(const KEY& Key)const 
{
	unsigned int i = 0;
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
unsigned int SKMap<KEY,VALUE,MMFun>::FindValueIndex(const VALUE &Value)const
{
	unsigned int i = 0;
	for(i = 0 ; i < m_uiCurUse ; i++)
	{
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
SKMapOrder<KEY,VALUE,MMFun>::~SKMapOrder()
{

}

template<class KEY,class VALUE,SKMemManagerFun MMFun>
SKMapOrder<KEY,VALUE,MMFun>::SKMapOrder(unsigned int uiGrowBy)
{


}
template<class KEY,class VALUE,SKMemManagerFun MMFun>
unsigned int SKMapOrder<KEY,VALUE,MMFun>::FindElement(unsigned int uiIndex0,unsigned int uiIndex1,const KEY& Key)const
{
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
template<class KEY1,class VALUE1>
unsigned int SKMapOrder<KEY,VALUE,MMFun>::Process(unsigned int uiIndex0,unsigned int uiIndex1,const MapElement<KEY1,VALUE1> & Element)
{
	if (uiIndex0 == uiIndex1)
	{
		return uiIndex0;
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
void SKMapOrder<KEY,VALUE,MMFun>::AddElement(const KEY& Key , const VALUE &Value)
{
	MapElement<KEY,VALUE> Element;
	Element.Key = Key;
	Element.Value = Value;
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
template<class KEY1,class VALUE1>
void SKMapOrder<KEY,VALUE,MMFun>::AddElement(const MapElement<KEY1,VALUE1> & Element)
{
	if(Find(Element.Key) != m_uiCurUse)
		return ;
	if(m_uiCurUse == m_uiBufferNum)
	...
	{
		SK_NEW (m_pBuffer + uiIndex) MapElement<KEY,VALUE>(Element);
		
	}
	else
	{
		SK_NEW (m_pBuffer + m_uiCurUse) MapElement<KEY,VALUE>(m_pBuffer[m_uiCurUse - 1]);
...
template<class KEY,class VALUE,SKMemManagerFun MMFun>
unsigned int SKMapOrder<KEY,VALUE,MMFun>::Find(const KEY& Key)const
{
	if (m_uiCurUse)
	{
