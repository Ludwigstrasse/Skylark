...
#define	DELEGATE_JOIN_MACRO(X,Y) DELEGATE_DO_JOIN(X,Y)
	...
	#define DELEGATE_CLASS_NAME SKDelegate
	#define EVENT_CLASS_NAME	SKDelegateEvent
#else
	#define DELEGATE_CLASS_NAME DELEGATE_JOIN_MACRO(SKDelegate,DELEGATE_PARAM_COUNT)
	#define EVENT_CLASS_NAME DELEGATE_JOIN_MACRO(SKDelegateEvent,DELEGATE_PARAM_COUNT)
#endif
...
private:
	typedef SKArray<Handler> DelegateList;
