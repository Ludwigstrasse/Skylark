...
#ifdef SKDATESTRUCT_EXPORTS
#define SKDATESTRUCT_API __declspec(dllexport) 
#else 
#define SKDATESTRUCT_API __declspec(dllimport) 
#endif
namespace SKEngine2
{
	template<class KEY, class VALUE>
	class MapElement : public SKMemObject
	{
	public:
		KEY		Key;
		VALUE	Value;
	...
	void MergeSort(MapElement<KEY, VALUE> *pBuffer, MapElement<KEY, VALUE> *TempBuffer, unsigned int  uiBufferLength,N Compare)
	{
		...
		if ((uiNumSwap % 2) != 0)
		{
			SKMemcpy(TempBuffer, pBuffer, sizeof(MapElement<KEY, VALUE>)* uiBufferLength);
		}
	}
	template <class KEY, class VALUE>
	...
	void MergeSort(T *pBuffer, T *TempBuffer, unsigned int  uiBufferLength)
	{
		...
		if ((uiNumSwap % 2) != 0)
		{
			SKMemcpy(TempBuffer, pBuffer, sizeof(T)* uiBufferLength);
		}
	}
	...
	class SKContainer : public SKMemObject
	{
	protected:
