...
IMPLEMENT_RTTI(SKMinnaertShaderFunction,SKShaderMainFunction)
BEGIN_ADD_PROPERTY(SKMinnaertShaderFunction,SKShaderMainFunction)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKMinnaertShaderFunction)
IMPLEMENT_INITIAL_END
SKMinnaertShaderFunction::SKMinnaertShaderFunction(const SKUsedName &ShowName,SKMaterial * pMaterial)
...
bool SKMinnaertShaderFunction::GetFuntionString(SKString &OutString)const
{
	...
	else
	{
		...
		for (unsigned int i = 0 ; i < SKLight::LT_MAX ; i++)
		{
			...
			{			
				...
				SKRenderer::ms_pRenderer->MinnaertDirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_POWER]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
			...
			else if(i == SKLight::LT_POINT)
			{
				...
				SKRenderer::ms_pRenderer->MinnaertPointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_POWER]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);

			}
			else if(i == SKLight::LT_SPOT)
			{

				SKRenderer::ms_pRenderer->MinnaertSpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_POWER]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
...
SKMinnaertShaderFunction::~SKMinnaertShaderFunction()
{
