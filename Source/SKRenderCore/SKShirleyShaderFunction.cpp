...
IMPLEMENT_RTTI(SKShirleyShaderFunction,SKShaderMainFunction)
BEGIN_ADD_PROPERTY(SKShirleyShaderFunction,SKShaderMainFunction)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKShirleyShaderFunction)
IMPLEMENT_INITIAL_END
SKShirleyShaderFunction::SKShirleyShaderFunction(const SKUsedName &ShowName,SKMaterial * pMaterial)
...
bool SKShirleyShaderFunction::GetFuntionString(SKString &OutString)const
{
	...
	else
	{
		...
		for (unsigned int i = 0 ; i < SKLight::LT_MAX ; i++)
		{
			...
			{			
				...
				SKRenderer::ms_pRenderer->ShirleyDirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(),
					...
					SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
			...
			else if(i == SKLight::LT_POINT)
			{
				...
				SKRenderer::ms_pRenderer->ShirleyPointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_NU]->GetNodeName().GetString(),m_pInput[IN_NV]->GetNodeName().GetString(),m_pInput[IN_RS]->GetNodeName().GetString(),
					SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);

			}
			else if(i == SKLight::LT_SPOT)
			{

				SKRenderer::ms_pRenderer->ShirleySpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_NU]->GetNodeName().GetString(),m_pInput[IN_NV]->GetNodeName().GetString(),m_pInput[IN_RS]->GetNodeName().GetString(),
					SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
...
SKShirleyShaderFunction::~SKShirleyShaderFunction()
{
