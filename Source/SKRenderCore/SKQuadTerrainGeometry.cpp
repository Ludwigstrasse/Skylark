...
IMPLEMENT_RTTI(SKQuadTerrainGeometry,SKCLodTerrainGeometry)
BEGIN_ADD_PROPERTY(SKQuadTerrainGeometry,SKCLodTerrainGeometry)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKQuadTerrainGeometry)
IMPLEMENT_INITIAL_END
SKQuadTerrainGeometry::SKQuadTerrainGeometry()
{
	m_uiCurLevel = 0;
}
SKQuadTerrainGeometry::~SKQuadTerrainGeometry()
...
void SKQuadTerrainGeometry::ComputeVariance(unsigned int uiCurLevel)
{
	...
	if (m_Variance.GetNum() != uiVarianceLength * uiVarianceLength)
	{
		...
		SKMemset(&m_Variance[0],0,m_Variance.GetNum());
	}
	
	RecursiveComputeVariance(uiCurLevel);
}
bool SKQuadTerrainGeometry::RecursiveComputeVariance(unsigned int uiLevel)
{
	SKCLodTerrainNode * pTerrainNode = (SKCLodTerrainNode *)m_pParent;
	...
	for (unsigned int i = 0 ; i < (unsigned int)(1 << uiLevel) ; i++)
	{
		...
		{
			//iϵ(i << uiBias)jϵ(j << uiBias)(1 << pTerrainNode->GetTesselationLevel() + 1)
			...
			else
			{
				...
				if (m_pNeighbor[NT_LEFT])
				{
					...
					fVarianceI[6] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_LEFT])->m_Variance[(uiLNTopA + uiLNTopC) >> 1];
					fVarianceI[7] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_LEFT])->m_Variance[(uiLNBottomA + uiLNBottomC) >> 1];
				}
				//top two child
				if (m_pNeighbor[NT_TOP])
				{
					...
					fVarianceI[8] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_TOP])->m_Variance[(uiTNLeftA + uiTNLeftC) >> 1];
					fVarianceI[9] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_TOP])->m_Variance[(uiTNRightA + uiTNRightC) >> 1];
				}
				//right two child
				if (m_pNeighbor[NT_RIGHT])
				{
					...
					unsigned int uiLNBottomC = ((uiCLBegin + 1) << uiCLBias) * uiVarianceLength + (uiCLEnd << uiCLBias);
 					fVarianceI[10] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_RIGHT])->m_Variance[(uiLNTopA + uiLNTopC) >> 1];
					fVarianceI[11] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_RIGHT])->m_Variance[(uiLNBottomA + uiLNBottomC) >> 1];
				}
				//bottom two child
				if (m_pNeighbor[NT_BOTTOM])
				{
					...
					fVarianceI[12] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_BOTTOM])->m_Variance[(uiTNLeftA + uiTNLeftC) >> 1];
					fVarianceI[13] = ((SKQuadTerrainGeometry *)m_pNeighbor[NT_BOTTOM])->m_Variance[(uiTNRightA + uiTNRightC) >> 1];
				}
...
void SKQuadTerrainGeometry::LinkNeighbor()
{

}
void SKQuadTerrainGeometry::ClearInfo()
{
	m_uiCurLevel = 0;
}
void SKQuadTerrainGeometry::Tessellate(const SKVector3 & CameraPos)
{
	SKCLodTerrainNode * pTerrainNode = (SKCLodTerrainNode *)m_pParent;
...
void SKQuadTerrainGeometry::RecursiveTessellate(const SKVector3 & CameraPos,unsigned int uiLevel,unsigned int A,unsigned int B,
						   unsigned int C,unsigned int D)
{
	...
	SKVector3 Edge = pVer[A] - pVer[B];
	SKREAL fRatio = Dist.GetSqrLength() / (Edge.GetSqrLength() * Max(m_Variance[uiMiddle],1.0f) * pTerrainNode->GetCLODScale());
	if (fRatio < 1.0f)
	{
...
*/
unsigned int SKQuadTerrainGeometry::UpdateGeometry()
{
	
	return UGRI_END;
}
void SKQuadTerrainGeometry::UpdateOther(double dAppTime)
{
	if (!m_pMeshDate)
	{
		return;
	}
	struct SKQuadTerrainUpdateGeometryPara
	{
		unsigned int uiTesselationLevel;
		SKQuadTerrainGeometry * pTG;
		unsigned int m_uiCurLevel;
		unsigned int m_NeighborLevel[SKCLodTerrainGeometry::NT_MAX];
		SKQuadTerrainGeometry * pNeightbor[SKCLodTerrainGeometry::NT_MAX];
	};
	SKQuadTerrainUpdateGeometryPara QuadTerrainUpdateGeometryPara;
	SKCLodTerrainNode * pTerrainNode = (SKCLodTerrainNode *)m_pParent;
	...
	for (unsigned int i = 0 ; i < SKCLodTerrainGeometry::NT_MAX ;i++)
	{
		QuadTerrainUpdateGeometryPara.pNeightbor[i] = ((SKQuadTerrainGeometry *)m_pNeighbor[i]);
		if (QuadTerrainUpdateGeometryPara.pNeightbor[i])
		{
			QuadTerrainUpdateGeometryPara.m_NeighborLevel[i] = ((SKQuadTerrainGeometry *)m_pNeighbor[i])->m_uiCurLevel;
		}
	}
	ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(SKQuadTerrainUpdateGeometryCommand,
		SKQuadTerrainUpdateGeometryPara,QuadTerrainUpdateGeometryPara,QuadTerrainUpdateGeometryPara,
	{
		int iLevelDelt = 0; 

		SKUSHORT_INDEX * pIndexData = (SKUSHORT_INDEX *)QuadTerrainUpdateGeometryPara.pTG->GetMeshDate()->GetIndexBuffer()->Lock();
		...
		for (unsigned int i = 1 ; i < uiLength - 1; i++)
		{
			for(unsigned int j = 1 ; j < uiLength - 1; j++)
			{
				...
				SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_ALL,uiCurRenderTriNum);
			}
		}
		if (QuadTerrainUpdateGeometryPara.pNeightbor[NT_LEFT])
		{
			...
			if (iLevelDelt >= 0 && iLevelDelt == 1)//д == 1 ʵͿ
			{
				...
				for (unsigned int j = 1 ; j < uiLength - 1 ; j ++)
				{
					...
					SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NLEFT,uiCurRenderTriNum);
				}
		...
		if (QuadTerrainUpdateGeometryPara.pNeightbor[NT_RIGHT])
		{
			...
			if (iLevelDelt >= 0 && iLevelDelt == 1)//д == 1 ʵͿ
			{
				...
				for (unsigned int j = 1 ; j < uiLength - 1 ; j ++)
				{
					...
					SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NRIGHT,uiCurRenderTriNum);
				}
		...
		if (QuadTerrainUpdateGeometryPara.pNeightbor[NT_TOP])
		{
			...
			if (iLevelDelt >= 0 && iLevelDelt == 1)//д == 1 ʵͿ
			{
				...
				for (unsigned int i = 1 ; i < uiLength - 1 ; i ++)
				{
					...
					SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NTOP,uiCurRenderTriNum);
				}
		...
		if (QuadTerrainUpdateGeometryPara.pNeightbor[NT_BOTTOM])
		{
			...
			if (iLevelDelt >= 0 && iLevelDelt == 1) //д == 1 ʵͿ
			{
				...
				for (unsigned int i = 1 ; i < uiLength - 1 ; i ++)
				{
					...
					SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NBOTTOM,uiCurRenderTriNum);
				}
		...
		//left top
		{
			...
			{
				SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NTOP | AT_NLEFT,uiCurRenderTriNum);
			}
		...
		//right top
		{
			...
			{
				SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NTOP | AT_NRIGHT,uiCurRenderTriNum);
			}
		...
		//left bottom
		{
			...
			{
				SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NBOTTOM | AT_NLEFT,uiCurRenderTriNum);
			}
		...
		//rigth bottom
		{
			...
			{
				SKQuadTerrainGeometry::AddTriangle(pIndexData,A,B,C,D,AT_NBOTTOM | AT_NRIGHT,uiCurRenderTriNum);
			}
...
void SKQuadTerrainGeometry::AddTriangle(SKUSHORT_INDEX * &pIndexData,unsigned int A,unsigned int B,
