...
IMPLEMENT_RTTI(SKAnisotropicWardShaderFunction,SKShaderMainFunction)
BEGIN_ADD_PROPERTY(SKAnisotropicWardShaderFunction,SKShaderMainFunction)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKAnisotropicWardShaderFunction)
IMPLEMENT_INITIAL_END
SKAnisotropicWardShaderFunction::SKAnisotropicWardShaderFunction(const SKUsedName &ShowName,SKMaterial * pMaterial)
...
bool SKAnisotropicWardShaderFunction::GetFuntionString(SKString &OutString)const
{
	...
	else
	{
		...
		OutString += SKRenderer::ms_pRenderer->Float3() +  _T("WorldCameraDir")  + _T(" = ");
		...
		for (unsigned int i = 0 ; i < SKLight::LT_MAX ; i++)
		{
			...
			{			
				...
				SKRenderer::ms_pRenderer->AnisotropicWardDirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSX]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSY]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
			...
			else if(i == SKLight::LT_POINT)
			{
				...
				SKRenderer::ms_pRenderer->AnisotropicWardPointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSX]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSY]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);

			}
			else if(i == SKLight::LT_SPOT)
			{

				SKRenderer::ms_pRenderer->AnisotropicWardSpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSX]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSY]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
...
SKAnisotropicWardShaderFunction::~SKAnisotropicWardShaderFunction()
{
