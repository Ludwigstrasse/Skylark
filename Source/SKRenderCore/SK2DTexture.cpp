...
IMPLEMENT_RTTI(VS2DTexture,SKTexture)
BEGIN_ADD_PROPERTY(VS2DTexture,SKTexture)
END_ADD_PROPERTY
...
bool VS2DTexture:: SaveToFile(TCHAR * pSaveName)
{
	...
	unsigned char *pBufferTemp = NULL,*pBuffer = NULL;					//ʱ
	pBufferTemp = SK_NEW unsigned char[uiHightSize];
	pBuffer = GetBuffer(0);
	if(!pBufferTemp)
		return 0;
	...
	for (unsigned int index=0; index < m_uiHeight; index++)
		SKMemcpy(&pBufferTemp[((m_uiHeight - 1) - index) * bytes_per_line],
	...
	if(!pFile->Open(pSaveName,SKFile::OM_WB))
	{
		SKMAC_DELETE(pFile);
		return 0;
	}
...
VS2DTexture::VS2DTexture(unsigned int uiWidth, unsigned int uiHeight,
						...
						:SKTexture(uiFormatType,uiWidth,uiHeight,1,uiMipLevel,bIsStatic)
{
	SetMipLevel();
...
VS2DTexture::VS2DTexture(unsigned int uiWidth, unsigned int uiHeight,unsigned int uiFormatType,unsigned int uiMipLevel)
			:SKTexture(uiFormatType,uiWidth,uiHeight,1,uiMipLevel,true)
{
	//SetMipLevel();
