...
IMPLEMENT_RTTI(SKPhoneShaderFunction,SKShaderMainFunction)
BEGIN_ADD_PROPERTY(SKPhoneShaderFunction,SKShaderMainFunction)
REGISTER_ENUM_PROPERTY(m_uiSpecularType, SpecularType, PhoneShaderFunctionSpecularType, SKProperty::F_SAVE_LOAD_CLONE | SKProperty::F_REFLECT_NAME)
BEGIN_ADD_ENUM
ADD_ENUM(PhoneShaderFunctionSpecularType,ST_BlinnPhong)
ADD_ENUM(PhoneShaderFunctionSpecularType, ST_Phong)
END_ADD_ENUM
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKPhoneShaderFunction)
IMPLEMENT_INITIAL_END
SKPhoneShaderFunction::SKPhoneShaderFunction(const SKUsedName &ShowName,SKMaterial * pMaterial)
...
bool SKPhoneShaderFunction::GetFuntionString(SKString &OutString)const
{
	...
	else
	{
		...
		for (unsigned int i = 0 ; i < SKLight::LT_MAX ; i++)
		{
			...
			{			
				...
				SKRenderer::ms_pRenderer->DirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_SPECULAR_POW]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
			...
			else if(i == SKLight::LT_POINT)
			{
				...
				SKRenderer::ms_pRenderer->PointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_SPECULAR_POW]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);

			}
			else if(i == SKLight::LT_SPOT)
			{

				SKRenderer::ms_pRenderer->SpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_SPECULAR_POW]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
...
SKPhoneShaderFunction::~SKPhoneShaderFunction()
{
