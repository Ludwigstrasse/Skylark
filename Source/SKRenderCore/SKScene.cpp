#include "SKScene.h"
#include "SKResourceManager.h"
#include "SKGraphicInclude.h"
#include "SKUpdateThread.h"
#include "SKStream.h"
#include "SKSceneManager.h"
#include "SKMeshComponent.h"
using namespace SKEngine2;
IMPLEMENT_RTTI(SKScene,SKObject)
BEGIN_ADD_PROPERTY(SKScene,SKObject)
REGISTER_PROPERTY(m_pDynamic, Dynamic, SKProperty::F_SAVE_LOAD_COPY)
REGISTER_PROPERTY(m_pStaticRoot, StaticRoot, SKProperty::F_SAVE_LOAD_COPY)
REGISTER_PROPERTY(m_ObjectNodes, ObjectNodes, SKProperty::F_SAVE_LOAD_COPY)
REGISTER_PROPERTY(m_bIsBuild, IsBuild, SKProperty::F_SAVE_LOAD_COPY)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKScene)
IMPLEMENT_INITIAL_END
DECLEAR_TIME_PROFILENODE(ComputeVisibleSet, ViewFamilyUpdate)
#define MAX_NUM 200

SKScene::SKScene()
{

	m_pAllLight.Clear();
	m_pAllCamera.Clear();
	m_pStaticRoot = NULL;
	SKSceneManager::ms_pSceneManager->AddScene(this);
	m_bIsBuild = false;
}
SKScene::~SKScene()
{

	SKSceneManager::ms_pSceneManager->DeleteScene(this);
	m_pAllLight.Clear();
	m_pAllCamera.Clear();

	m_pDynamic.Clear();
	m_pStaticRoot = NULL;
	m_ObjectNodes.Clear();
	
}
void SKScene::Update(double dAppTime)
{
	if (m_bIsBuild == true)
	{
		struct UpdateNode
		{
			SKNode * pSpatial;
			double dAppTime;
		};
	
		if (m_pStaticRoot)
		{
			UpdateNode StaticUpdateNode;
			StaticUpdateNode.pSpatial = m_pStaticRoot;
			StaticUpdateNode.dAppTime = dAppTime;
			ENQUEUE_UNIQUE_UPDATE_COMMAND_ONEPARAMETER(SKUpdateStaticNode,
				UpdateNode, StaticUpdateNode, StaticUpdateNode,
				{
				StaticUpdateNode.pSpatial->UpdateAll(StaticUpdateNode.dAppTime);
			})
				StaticUpdateNode.pSpatial->UpdateAll(StaticUpdateNode.dAppTime);
			ENQUEUE_UNIQUE_RENDER_COMMAND_END
		}

		for (unsigned int i = 0; i < m_pDynamic.GetNum(); i++)
		{
			UpdateNode DynamicUpdateNode;
			DynamicUpdateNode.pSpatial = m_pDynamic[i];
			DynamicUpdateNode.dAppTime = dAppTime;
			ENQUEUE_UNIQUE_UPDATE_COMMAND_ONEPARAMETER(SKUpdateDynamicNode,
				UpdateNode, DynamicUpdateNode, DynamicUpdateNode,
				{
				DynamicUpdateNode.pSpatial->UpdateAll(DynamicUpdateNode.dAppTime);
			})
				DynamicUpdateNode.pSpatial->UpdateAll(DynamicUpdateNode.dAppTime);
			ENQUEUE_UNIQUE_RENDER_COMMAND_END
		}
	}
	else
	{
		for (unsigned int i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			m_ObjectNodes[i]->UpdateAll(dAppTime);
		}
	}

}
void SKScene::CollectUpdateInfo()
{
	if(m_pAllCamera.GetNum() > 0)
		m_pAllCamera.Clear();
	if(m_pAllLight.GetNum() > 0)
		m_pAllLight.Clear();
	if (m_bIsBuild == false)
	{

		for (unsigned int i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			if (m_ObjectNodes[i])
			{
				if (m_ObjectNodes[i]->m_pAllLight.GetNum() > 0)
					m_pAllLight.AddElement(m_ObjectNodes[i]->m_pAllLight, 0, m_ObjectNodes[i]->m_pAllLight.GetNum() - 1);
			}
		}
		for (unsigned int i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			if (m_ObjectNodes[i])
			{
				if (m_ObjectNodes[i]->m_pAllCamera.GetNum() > 0)
					m_pAllCamera.AddElement(m_ObjectNodes[i]->m_pAllCamera, 0, m_ObjectNodes[i]->m_pAllCamera.GetNum() - 1);
			}
		}
	}
	else
	{
		if (m_pStaticRoot)
		{
			if (m_pStaticRoot->m_pAllCamera.GetNum() > 0)
				m_pAllCamera.AddElement(m_pStaticRoot->m_pAllCamera, 0, m_pStaticRoot->m_pAllCamera.GetNum() - 1);
		}


		for (unsigned int i = 0; i < m_pDynamic.GetNum(); i++)
		{
			if (m_pDynamic[i])
			{
				if (m_pDynamic[i]->m_pAllCamera.GetNum() > 0)
					m_pAllCamera.AddElement(m_pDynamic[i]->m_pAllCamera, 0, m_pDynamic[i]->m_pAllCamera.GetNum() - 1);
			}

		}

		if (m_pStaticRoot)
		{
			if (m_pStaticRoot->m_pAllLight.GetNum() > 0)
				m_pAllLight.AddElement(m_pStaticRoot->m_pAllLight, 0, m_pStaticRoot->m_pAllLight.GetNum() - 1);
		}



		for (unsigned int i = 0; i < m_pDynamic.GetNum(); i++)
		{
			if (m_pDynamic[i])
			{
				if (m_pDynamic[i]->m_pAllLight.GetNum() > 0)
					m_pAllLight.AddElement(m_pDynamic[i]->m_pAllLight, 0, m_pDynamic[i]->m_pAllLight.GetNum() - 1);
			}


		}
	}
}
void SKScene::AddObject(SKNode * pObject)
{
	if (pObject)
	{
		m_ObjectNodes.AddElement(pObject);
	}
	if (m_bIsBuild)
	{
		m_pDynamic.AddElement(pObject);
	}
}
void SKScene::DeleteObject(SKNode *pObject)
{
	
	if (!pObject)
	{
		
		for (unsigned int i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			if (pObject == m_ObjectNodes[i])
			{
				m_ObjectNodes.Erase(i);
				if (m_bIsBuild)
				{
					for (unsigned int j = 0; j < m_pDynamic.GetNum(); j++)
					{
						if (pObject == m_pDynamic[i])
						{
							m_pDynamic.Erase(j);
							return;
						}
					}
					SKNode * pSpatial = DynamicCast<SKNode>(pObject->GetParent());
					if (pSpatial)
					{
						pSpatial->DeleteChild(pObject);
					}
					return;
				}
			}
		}
		
	}
}

void SKScene::DeleteAllObject()
{
	m_pDynamic.Clear();
	m_pStaticRoot = NULL;
	m_ObjectNodes.Clear();
}

void SKScene::ComputeVisibleSet(SKCuller & Culler,bool bNoCull,double dAppTime)
{
	ADD_TIME_PROFILE(ComputeVisibleSet);
	if (m_bIsBuild == false)
	{
		for (unsigned int i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			if (m_ObjectNodes[i])
			{

				m_ObjectNodes[i]->ComputeVisibleSet(Culler, bNoCull, dAppTime);
			}

		}
	}
	else
	{
		if (m_pStaticRoot)
		{

			m_pStaticRoot->ComputeVisibleSet(Culler, bNoCull, dAppTime);

		}
		for (unsigned int i = 0; i < m_pDynamic.GetNum(); i++)
		{
			if (m_pDynamic[i])
			{

				m_pDynamic[i]->ComputeVisibleSet(Culler, bNoCull, dAppTime);
			}

		}
	}
	
}
//add loaded Scene
bool SKScene::Build()
{	
	m_pDynamic.Clear();
	m_pStaticRoot = NULL;
	SKArray<SKSpatial *> pStatic;
	for (unsigned int i = 0; i < m_ObjectNodes.GetNum();i++)
	{
		m_ObjectNodes[i]->UpdateAll(0);
		SKMeshComponent * pMesh = DynamicCast<SKMeshComponent>(m_ObjectNodes[i]);
		if (pMesh && !pMesh->IsDynamic())
		{
			pStatic.AddElement(m_ObjectNodes[i]);
		}
		else
		{
			m_pDynamic.AddElement(m_ObjectNodes[i]);
		}
	}
	m_pStaticRoot = SK_NEW SKQuadNode();
	if (!m_pStaticRoot->RecursiveBuild(pStatic))
	{
		m_pStaticRoot = NULL;
	}
	else
	{
		m_pStaticRoot->UpdateAll(0.0f);
	}
	
	CollectUpdateInfo();
	for (unsigned int i = 0; i < m_pAllLight.GetNum(); i++)
	{
		m_pAllLight[i]->BuildSceneInfo(this);
	}
	m_bIsBuild = true;
	return 1;
}
IMPLEMENT_RTTI(SKQuadNode,SKNode)
BEGIN_ADD_PROPERTY(SKQuadNode,SKNode)
REGISTER_PROPERTY(m_pNeedDrawNode, NeedDrawNode, SKProperty::F_SAVE_LOAD_CLONE)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKQuadNode)
IMPLEMENT_INITIAL_END
SKQuadNode::SKQuadNode()
{

}
SKQuadNode::~SKQuadNode()
{

}
unsigned int SKQuadNode::AddChild(SKSpatial * pChild)
{
	unsigned int id = SKNode::AddChild(pChild);
	SKNodeComponent * pNode = DynamicCast<SKNodeComponent>(pChild);
	if (pNode &&  pNode->IsNeedDraw())
	{
		AddNeedDrawNode(pNode);
	}
	return id;
}
unsigned int SKQuadNode::DeleteChild(SKSpatial *pChild)
{
	unsigned int id = SKNode::DeleteChild(pChild);

	SKNodeComponent * pNode = DynamicCast<SKNodeComponent>(pChild);
	if (pNode &&  pNode->IsNeedDraw())
	{
		DeleteNeedDrawNode(pNode);
	}
	return id;
}
bool SKQuadNode::DeleteChild(unsigned int i)
{
	if (i >= m_pChild.GetNum())
		return 0;
	SKSpatial * Temp = m_pChild[i];
	SKNodeComponent * pNode = DynamicCast<SKNodeComponent>(Temp);
	if (pNode && pNode->IsNeedDraw())
	{
		DeleteNeedDrawNode(pNode);
	}
	return SKNode::DeleteChild(i);
}
void SKQuadNode::DeleteAllChild()
{
	for (unsigned int i = 0; i < m_pChild.GetNum(); i++)
	{
		SKNodeComponent * pNode = DynamicCast<SKNodeComponent>(m_pChild[i]);
		if (pNode&& pNode->IsNeedDraw())
		{
			DeleteNeedDrawNode(pNode);
		}
	}
	SKNode::DeleteAllChild();
}
SKNodeComponent *SKQuadNode::GetNeedDrawNode(unsigned int uiIndex)const
{
	if (uiIndex < m_pNeedDrawNode.GetNum())
	{
		return m_pNeedDrawNode[uiIndex];
	}
	return NULL;
}
unsigned int SKQuadNode::GetNeedDrawNodeNum()const
{
	return m_pNeedDrawNode.GetNum();
}

void SKQuadNode::ComputeNodeVisibleSet(SKCuller & Culler, bool bNoCull, double dAppTime)
{
	
	if (bNoCull && m_pNeedDrawNode.GetNum() > 0)
	{
		for (unsigned int i = 0; i < m_pNeedDrawNode.GetNum(); i++)
		{
			if (m_pNeedDrawNode[i])
			{
				m_pNeedDrawNode[i]->ComputeVisibleSet(Culler, bNoCull, dAppTime);
			}
		}
	}
	else
	{
		SKNode::ComputeNodeVisibleSet(Culler, bNoCull, dAppTime);
	}


}
void SKQuadNode::AddNeedDrawNode(SKNodeComponent *pNeedDrawNode)
{


	SKQuadNode * pNode = DynamicCast<SKQuadNode>(m_pParent);

	if (pNode)
	{
		pNode->AddNeedDrawNode(pNeedDrawNode);
	}
	unsigned int i = m_pNeedDrawNode.FindElement(pNeedDrawNode);
	if (i >= m_pNeedDrawNode.GetNum())
	{
		m_pNeedDrawNode.AddElement(pNeedDrawNode);
	}


}
void SKQuadNode::DeleteNeedDrawNode(SKNodeComponent *pNeedDrawNode)
{

	SKQuadNode * pNode = DynamicCast<SKQuadNode>(m_pParent);
	if (pNode)
	{
		pNode->DeleteNeedDrawNode(pNeedDrawNode);
	}
	unsigned int i = m_pNeedDrawNode.FindElement(pNeedDrawNode);
	if (i < m_pNeedDrawNode.GetNum())
	{
		m_pNeedDrawNode.Erase(i);
	}

}
bool SKQuadNode::RecursiveBuild(const SKArray<SKSpatial *> &pObjectArray)
{
	if (!pObjectArray.GetNum())
	{
		return 0;
	}
	//ݹ
	if (pObjectArray.GetNum() < MAX_NUM)
	{
		for (unsigned int i = 0 ; i < pObjectArray.GetNum() ; i++)
		{
			AddChild(pObjectArray[i]);
		}
		return 1;
	}
	//ǰнڵBounds
	bool bFound = false;
	SKAABB3 Total;
	for (unsigned int i = 0 ; i < pObjectArray.GetNum() ; i++)
	{
		if (pObjectArray[i])
		{
			const SKAABB3 & WorldAABB = pObjectArray[i]->GetWorldAABB();

			if (!bFound)
			{
				Total = WorldAABB;
				bFound = true;
			}
			else
			{
				Total = Total.MergAABB(WorldAABB);
			}

		}
	}
	//ĸȻڵ
	SKAABB3 ChildAABB[4];
	Total.GetQuadAABB(ChildAABB);
	SKArray<SKSpatial *> ChildStatic[4];
	for (unsigned int i = 0 ; i < pObjectArray.GetNum() ; i++)
	{
		if (!pObjectArray[i])
		{
			continue;
		}
		for (unsigned int j = 0 ; j < 4 ; j++)
		{
			if (ChildAABB[j].RelationWith(pObjectArray[i]->GetWorldTranslate()) != SKOUT)
			{
				ChildStatic[j].AddElement(pObjectArray[i]);
				break;
			}
		}
	}
	//ݹ
	for (unsigned int i = 0 ; i < 4 ; i++)
	{
		if (ChildStatic[i].GetNum())
		{
			SKQuadNode * pQuadNode = SK_NEW SKQuadNode();
			AddChild(pQuadNode);
			if(!pQuadNode->RecursiveBuild(ChildStatic[i]))
				return 0;
		}
	}
	return 1;
}
FORCEINLINE SKLight * SKScene::GetAllLight(unsigned int i)const
{
	if (i >= m_pAllLight.GetNum())
	{
		return NULL;
	}
	return m_pAllLight[i];
}
FORCEINLINE unsigned int SKScene::GetAllLightNum()const
{
	return m_pAllLight.GetNum();
}
