...
IMPLEMENT_RTTI(SKSchlickShaderFunction,SKShaderMainFunction)
BEGIN_ADD_PROPERTY(SKSchlickShaderFunction,SKShaderMainFunction)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(SKSchlickShaderFunction)
IMPLEMENT_INITIAL_END
SKSchlickShaderFunction::SKSchlickShaderFunction(const SKUsedName &ShowName,SKMaterial * pMaterial)
...
bool SKSchlickShaderFunction::GetFuntionString(SKString &OutString)const
{
	...
	else
	{
		...
		for (unsigned int i = 0 ; i < SKLight::LT_MAX ; i++)
		{
			...
			{			
				...
				SKRenderer::ms_pRenderer->SchlickDirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
			...
			else if(i == SKLight::LT_POINT)
			{
				...
				SKRenderer::ms_pRenderer->SchlickPointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
			...
			else if(i == SKLight::LT_SPOT)
			{
				...
				SKRenderer::ms_pRenderer->SchlickSpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_SPECULAR_POW]->GetNodeName().GetString(),SKShaderStringFactory::ms_WorldPos,SKShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
...
SKSchlickShaderFunction::~SKSchlickShaderFunction()
{
