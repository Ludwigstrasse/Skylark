...
IMPLEMENT_RTTI(VS3DTexture,SKTexture)
BEGIN_ADD_PROPERTY(VS3DTexture,SKTexture)
END_ADD_PROPERTY
...
VS3DTexture::VS3DTexture(unsigned int uiWidth, unsigned int uiHeight,unsigned int uiLength,
						 ...
						 :SKTexture(uiFormatType,uiWidth,uiHeight,uiLength,uiMipLevel,bIsStatic)
{
...
void VS3DTexture::SetMipLevel()
{
	...
	SKMAC_ASSERT(IsTwoPower(m_uiWidth) && IsTwoPower(m_uiHeight) && IsTwoPower(m_uiLenght));
