...
unsigned int SKStream::ms_uiCurVerson = 0;
/********************************************************************************/
int SKStream::GetStrDistUse(const SKString &Str)
{
	return sizeof(int) + (unsigned int)Str.GetLength() * sizeof(TCHAR);

}
/********************************************************************************/
int SKStream::GetStrDistUse(const TCHAR * pCh)
{
	return sizeof(int) + (unsigned int)SKStrLen(pCh) * sizeof(TCHAR);
}
/********************************************************************************/
SKStream::SKStream()
{
...
/********************************************************************************/
bool SKStream::Read(void * pvBuffer,unsigned int uiSize)
{
	...
	SKMemcpy(pvBuffer,m_pcCurBufPtr,uiSize);
...
/********************************************************************************/
bool SKStream::Write(const void * pvBuffer,unsigned int uiSize)
{
	if(!pvBuffer || !m_pcBuffer)
		return 0;
	...
	SKMemcpy(m_pcCurBufPtr,pvBuffer,uiSize);

	m_pcCurBufPtr +=uiSize;
	return 1;
}
/********************************************************************************/
bool SKStream::ReadString(SKString & String)
{
...
#ifdef	_UNICODE		
		SKMemcpy(String.GetBuffer(),pCh,uiBufferSize);
#else
		SKWcsToMbs(String.GetBuffer(),uiStrLen,(wchar_t *)pCh,uiStrLen);
#endif
	}
	else
	{
		...
		String.SetTCHARBufferNum(uiStrLen);
#ifdef	_UNICODE
		SKMbsToWcs((wchar_t *)String.GetBuffer(),uiStrLen,pCh,uiStrLen);
#else
		SKMemcpy(String.GetBuffer(),pCh,uiBufferSize);
#endif
	}
...
/********************************************************************************/
bool SKStream::WriteString(const SKString & String)
{
...
bool SKStream::RegisterReachableObject(SKObject * pObject)
{
	if (pObject->IsHasFlag(SKObject::OF_REACH))
	{
		return false;
	}
	pObject->SetFlag(SKObject::OF_REACH);
	pObject->ClearFlag(SKObject::OF_UNREACH);

	return 1;
}
bool SKStream::RegisterPostLoadObject(SKObject * pObject)
{
	...
	m_pPostLoadObject.AddElement((SKObject *)pObject);	
	return true;
}

/********************************************************************************/
bool SKStream::RegisterObject(SKObject * pObject)
{
	SKMAC_ASSERT(pObject);
	if(!pObject)
		return 0;
	...
	m_pVObjectArray.AddElement((SKObject *)pObject);

	return 1;
}
/********************************************************************************/
SKObject *SKStream::GetVSTypeMapValue(SKObject *pKey)const
{
...
/********************************************************************************/
const SKObject *SKStream::GetMapValue(SKObject* pKey)const 
{
...
/********************************************************************************/
const SKObject *SKStream::GetObjectByRtti(const SKRtti &Rtti)
{
	SKObject * pObject = NULL;
	...
	if (pObject)
	{
		...
		GCCollectStream.SetStreamFlag(SKStream::AT_LOAD_OBJECT_COLLECT_GC);
		...
		SKResourceManager::AddCanGCObject(CanGCObject);
...
bool SKStream::GetAllResourceObject(SKArray<SKObject *> &ObjectArray)
{
	ObjectArray.Clear();
	for (unsigned int i = 0; i < m_pVObjectArray.GetNum(); i++)
	{
		if (SKResourceManager::IsResourceType(m_pVObjectArray[i]->GetType()))
		{
...
/********************************************************************************/
bool SKStream::GetObjectArrayByRtti(const SKRtti &Rtti, SKArray<SKObject *> &ObjectArray, bool IsDerivedFrom)
{
	ObjectArray.Clear();
	for(unsigned int i = 0 ; i < m_pVObjectArray.GetNum() ; i++)
	{
...
bool SKStream::WriteResource(SKResourceProxyBasePtr& Resource)
{
	bool IsNone = true;
	if (Resource)
	{
	...
	if (Resource)
	{
		...
		SKString FileName = Resource->GetResourceName().GetString();
...
bool SKStream::ReadResource(SKResourceProxyBasePtr& Resource)
{
	bool IsNone = true;
	...
	if (IsNone == false)
	{
		SKString Name;
		ReadString(Name);
		Resource = SKResourceManager::LoadResource(Name.GetBuffer(),true);
		if (!Resource)
		{
			SKMAC_ASSERT(0);
			return false;
...
int SKStream::GetResourceDistUse(SKResourceProxyBasePtr& Resource)
{
	if (Resource)
	{
		SKString FileName = Resource->GetResourceName().GetString();
...
bool SKStream::ArchiveAll(SKObject * pObject)
{
	if (!pObject)
	{
		return false;
	...
	if (m_uiStreamFlag == AT_REGISTER)
	{
		if(RegisterObject(pObject))
		{
			...
			{
				SKProperty * pProperty = Rtti.GetProperty(j);
				if (pProperty->GetFlag() & SKProperty::F_SAVE_LOAD)
				{
	...
	else if (m_uiStreamFlag == AT_OBJECT_COLLECT_GC)
	{
		if (RegisterReachableObject(pObject))
		{
			...
			{
				SKProperty * pProperty = Rtti.GetProperty(j);
				if (!(pProperty->GetFlag() & SKProperty::F_NO_USE_GC))
				{
					pProperty->Archive(*this, pObject);
				}
			}		
...
bool SKStream::NewLoadFromBuffer(unsigned char * pBuffer,unsigned int uiSize)
{
	...
	SKArray<ObjectTabel> ObjectTable;
	//ذ汾
	...
	for(unsigned int i = 0 ;i < iObjectNum ; i++)
	{
		SKObject * pObject = 0;
		//ȡָ
		if(!Read(&ObjectTable[i].m_pAddr,sizeof(SKObject *)))
		{
			return 0;
		}
		//ȡRTTI
	...
	for(unsigned int i = 0 ;i < iObjectNum ; i++)
	{
		...
		if (m_bLoadUseGC)
		{
			pObject = SKObject::GetInstance(ObjectTable[i].m_RttiName);
		}
		else
		{
			pObject = SKObject::GetNoGCInstance(ObjectTable[i].m_RttiName);
		}
		//ն
	...
	for (unsigned int i = 0; i < m_pVObjectArray.GetNum(); i++)
	{
		SKRtti &Rtti = m_pVObjectArray[i]->GetType();
	...
	//ӺǰΪregisterĹǵݹעᣬԴӺǰlink֤ӽڵɣȻ׽ڵɡ 
...
bool SKStream::NewLoad(const TCHAR * const pcFileName)
{
	SKFile *pFile = SK_NEW SKFile();
	if (!pFile)
	{
		return 0;
	}
	...
	//ļ
	if(!pFile->Open(pcFileName,SKFile::OM_RB))
	{
		SKMAC_DELETE(pFile);
		return 0;
	}
	...
	unsigned char * pBuffer = SK_NEW unsigned char[uiBufferSize];
	if(!pBuffer)
	{
		SKMAC_DELETE(pFile);
		return 0;
...
bool SKStream::NewSave(const TCHAR * const pcFileName)
{
	...
	{
		//addr
		m_uiBufferSize += sizeof(SKObject*);
	...
	//ռ
	SKMAC_DELETEA(m_pcBuffer);
	m_pcBuffer = SK_NEW unsigned char[m_uiBufferSize];	
	if(!m_pcBuffer)
	{
		return 0;
	}
	...
	{
		//addr
		if(!Write(&ObjectTable[i].m_pAddr,sizeof(SKObject *)))
		{
			SKMAC_DELETEA(m_pcBuffer);
			return 0;
		}
	...
	//ļ
	if(!pFile->Open(pcFileName,SKFile::OM_WB))
	{
		SKMAC_DELETE(pFile);
		SKMAC_DELETEA(m_pcBuffer);
		return 0;
