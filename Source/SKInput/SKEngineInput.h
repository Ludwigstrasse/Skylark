#ifndef SKENGINEINPUT_H
#define SKENGINEINPUT_H
#include "SKSystem.h"
#ifdef SKINPUT_EXPORTS
#define  SKINPUT_API __declspec(dllexport) 
#else
#define  SKINPUT_API __declspec(dllimport)
#endif
namespace SKEngine2
{
	typedef void (* InputMsgProc)(unsigned int uiInputType,unsigned int uiEvent,unsigned int uiKey,int x, int y, int z);
	class SKINPUT_API SKEngineInput
	{
	public:
		...
		static	SKEngineInput * ms_pInput;
	...
	protected:
		...
		MouseRange      m_Cage;							//귶Χ
		bool			m_bPressed[3];					//洢Ƿ
		bool			m_bReleased[3];					//жǷͷ
		MousePoint      m_Delta;							//ƫ
		int                m_lX;
		int                m_lY;
		int					m_lZ;

		//
		TCHAR  m_Keys[256];					//洢̼Ƿ
		TCHAR  m_KeysOld[256];				//һμ̼룬жϼǷͷ
