#ifndef SKSYNCHRONIZE_H
#define SKSYNCHRONIZE_H
#include "SKSystem.h"
namespace SKEngine2
{
	class SKSYSTEM_API SKSynchronize
	{
	public:
		SKSynchronize()
		{
		...
		static unsigned int WaitAll(SKSynchronize * * pSynchronize, unsigned int uiNum, bool bWaitAll,DWORD dwMilliseconds = (DWORD)-1);
		static void SKSafeOutPutDebugString(const TCHAR * pcString, ...);
	};

	class SKSYSTEM_API SKCriticalSection
	{
		/**
	...
	public:
		...
		*/
		FORCEINLINE SKCriticalSection(void)
		{
	...
	class SKSYSTEM_API SKSemaphore : public SKSynchronize
	{
	public:
		SKSemaphore (unsigned int uiCount,unsigned int MaxCount);
		virtual ~SKSemaphore ();
	...
	class SKSYSTEM_API SKMutex : public SKSynchronize
	{
	public:
	...
	class SKSYSTEM_API SKEvent : public SKSynchronize
	{
		/**
