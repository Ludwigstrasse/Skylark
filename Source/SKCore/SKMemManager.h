#ifndef SKMEMMANAGER_H
#define SKMEMMANAGER_H
#include "SKSystem.h"
...
namespace SKEngine2
{
	...
	*/
	...
	class SKSYSTEM_API SKMemManager
	{
	public:
		SKMemManager();
		virtual ~SKMemManager() = 0;
	...
	class SKSYSTEM_API SKCMem : public SKMemManager
	{
	public:
		SKCMem();
		~SKCMem();
	...
	class SKSYSTEM_API SKDebugMem : public SKMemManager
	{
	public:
		SKDebugMem();
		~SKDebugMem();
	...
	private:
		...
		static SKCriticalSection ms_MemLock;
		class Block
		{
		public:
			Block()
			...
			void * pAddr[CALLSTACK_NUM];	//ڴʱĵöջϢ
			unsigned int m_uiStackInfoNum;	//ջ
			unsigned int m_uiSize;			//ռĴС
			bool m_bIsArray;				//Ƿ
			bool m_bAlignment;				//Ƿֽڶ
			Block * m_pPrev;				//ǰһڵ
			Block * m_pNext;				//һڵ
		};
	...
	// visit my blog 
	class SKSYSTEM_API SKMemWin32 : public SKMemManager
	{
	public:
		SKMemWin32();
		~SKMemWin32();
	...
	class SKSYSTEM_API SKStackMem : public SKMemManager
	{
	public:
		SKStackMem(unsigned int uiDefaultChunkSize = 65536);
		~SKStackMem();
		void* Allocate (unsigned int uiSize,unsigned int uiAlignment,bool bIsArray);
		void Deallocate (char* pcAddr, unsigned int uiAlignment,bool bIsArray)
		{
	...
	// if the class has memory alloc , you must inherit from SKMemObject , so you can use the MemManger any where 
	class SKSYSTEM_API SKMemObject
	{
	public:
		SKMemObject();
		~SKMemObject();
		static SKStackMem& GetStackMemManager();
		static SKMemManager& GetMemManager();

	};
	typedef SKMemManager& (*SKMemManagerFun)();

	template<class T>
	class SKStackMemAlloc : public SKMemObject
	{
	public:
		// Constructors.
		SKStackMemAlloc(unsigned int uiNum = 0,unsigned int uiAlignment = 0)
		{
			m_uiNum = uiNum;
	...
	private:
		BYTE* Top;
		SKStackMem::FTaggedMemory* SavedChunk;
		T * m_pPtr;
		unsigned int m_uiNum;
	};
	// 	class SKMemManagerSF
	// 	{
	// 	public:
	// 		class BlockHeader
	// 		{
...
{
	return SKEngine2::SKMemObject::GetMemManager().Allocate((unsigned int)uiSize,0,false);
}
FORCEINLINE void* operator new[] (size_t uiSize)
{
	return SKEngine2::SKMemObject::GetMemManager().Allocate((unsigned int)uiSize,0,true);
}

FORCEINLINE void operator delete (void* pvAddr)
{
	return SKEngine2::SKMemObject::GetMemManager().Deallocate((char *)pvAddr,0,false);
}
FORCEINLINE void operator delete[] (void* pvAddr)
{
	return SKEngine2::SKMemObject::GetMemManager().Deallocate((char *)pvAddr,0,true);
}
#endif


#define SKMAC_DELETE(p) if(p){SK_DELETE p; p = 0;}
#define SKMAC_DELETEA(p) if(p){SK_DELETE []p; p = 0;}
#define SKMAC_DELETEAB(p,num) if(p){ for(int i = 0 ; i < num ; i++) SKMAC_DELETEA(p[i]); SKMAC_DELETEA(p);}
// use by inner mac
template<typename T>
FORCEINLINE void SKDelete(T * p)
{
	if (p){ SK_DELETE p; p = 0; }
}
template<typename T>
FORCEINLINE void SKDeleteA(T * p)
{
	if (p){ SK_DELETE[]p; p = 0; }
}
template<typename T,typename N>
FORCEINLINE void SKDeleteAB(T * p,N num)
{
	if (p){ for (int i = 0; i < num; i++) SKMAC_DELETEA(p[i]); SKMAC_DELETEA(p); }
}
#endif