...
namespace SKEngine2
{
SKCriticalSection SKMemWin32::ms_MemLock;
SKCriticalSection SKDebugMem::ms_MemLock;
// char *  SKMemManagerSF::ms_pMemHeapPtr = NULL;
// SKMemManagerSF::BlockHeader* SKMemManagerSF::ms_pFreeList = NULL;
// unsigned int SKMemManagerSF::ms_uiSize = 0;
// void SKMemManagerSF::CreateMemMangerSF(unsigned int uiSize)
// {
// 	ms_uiSize = ms_uiSize;
// 	ms_pMemHeapPtr =(char *) malloc(ms_uiSize);
// 	SKMAC_ASSERT(ms_pMemHeapPtr);
...
// }
// void SKMemManagerSF::DeleteMemMangerSF()
// {
// 	if(ms_pMemHeapPtr)
// 		free(ms_pMemHeapPtr);	
// 
// }
// void *SKMemManagerSF::SearchByPolicy(unsigned int uiRequestedSize)
// {
// 	if(!ms_pFreeList)
// 		return NULL;
...
// 
// }
// void *SKMemManagerSF::Allocate(unsigned int uiRequestedSize)
// {
...
// 
// 		/*ֻʣһ飬ֳ2飬pNewHeaderָ붼ҪָԼ,
// 		pHeader->m_pcPrev  pHeader
// 		pHeader->m_pcPrev->m_pcNext = pNewHeader pHeader->m_pcNext = pNewHeader;
// 
// 		pHeader->m_pcNext->m_pcPrev = pNewHeader; ൱pNewHeader->m_pcPrev = pNewHeader;
// 
// 		pNewHeader->m_pcNext = pHeader->m_pcNext;൱ pNewHeader->m_pcNext = pNewHeader;
// 		*/
// 
// 		if(pHeader == ms_pFreeList)
// 		{
...
// }
// void SKMemManagerSF::Deallocate(void * pcDelete)
// {
// 	BlockHeader * pHeader =(BlockHeader *)((unsigned int )pcDelete - HEADERSIZE);
// 
// 	BlockEnder *pEnder = (BlockEnder *)((unsigned int)pHeader + HEADERSIZE + pHeader->ms_uiSize);
// 
// 	SKMAC_ASSERT(pHeader->m_bUsed == pEnder->m_bUsed);
// 	SKMAC_ASSERT(pEnder->ms_uiSize == pHeader->ms_uiSize);
...
bool SKDebugMem::InitDbgHelpLib()
{
	TCHAR szDbgName[MAX_PATH];
	GetModuleFileName(NULL, szDbgName, MAX_PATH);
	TCHAR *p = (TCHAR *)SKCsrchr(szDbgName, _T('\\'));
	if(p)
		*p = 0;
	SKStrcat(szDbgName,MAX_PATH,_T("\\SKdbghelp.dll"));
	...
	{
		// ʹϵͳDLL
		s_DbgHelpLib = LoadLibrary(_T("SKdbghelp.dll"));
		if(s_DbgHelpLib == NULL)
			return false;
	}
...
void SKDebugMem::FreeDbgHelpLib()
{
	if(s_DbgHelpLib != NULL)
	{
...
bool SKDebugMem::GetFileAndLine(const void *pAddress, TCHAR szFile[MAX_PATH], int &line)
{

	IMAGEHLP_LINE64 Line;
	Line.SizeOfStruct = sizeof(Line);
	SKMemset(&Line, 0, sizeof(Line));
	DWORD Offset = 0;

	if(fnSymGetLineFromAddr64(s_Process, (DWORD64)pAddress, &Offset, &Line))
	{
#ifdef  _UNICODE
	SKMbsToWcs(szFile,MAX_PATH,Line.FileName,MAX_PATH);
#else
	SKStrCopy(szFile, MAX_PATH,Line.FileName);
#endif
...
// 		if (error == 487)
// 		{
// 			SKOutPutDebugString(_T("No debug info in current module\n"));
// 		}
// 		else if (error == 126)
// 		{
// 			SKOutPutDebugString(_T("Debug info in current module has not loaded\n"));
// 		}
// 		else 
// 		{
// 			SKOutPutDebugString(_T("SymGetLineFromAddr64 failed\n"));
// 		}
		return false;
	}
}
SKMemManager::SKMemManager()
{
...
void* SKDebugMem::Allocate (unsigned int uiSize,unsigned int uiAlignment,bool bIsArray)
{
...
void SKDebugMem::Deallocate (char* pcAddr,unsigned int uiAlignment, bool bIsArray)
{
	ms_MemLock.Lock();
	m_uiNumDeleteCalls++;
	...
	unsigned int *pBeginMask = (unsigned int *)(pcAddr);
	SKMAC_ASSERT(*pBeginMask == BEGIN_MASK);
	...
	SKMAC_ASSERT(pBlock->m_bIsArray == bIsArray);
	SKMAC_ASSERT(m_uiNumBlocks > 0 && m_uiNumBytes >= pBlock->m_uiSize);
	bool bAlignment = (uiAlignment > 0) ? true : false;
	SKMAC_ASSERT(pBlock->m_bAlignment == bAlignment);
	unsigned int * pEndMask = (unsigned int *)(pcAddr + sizeof(Block) + sizeof(unsigned int) + pBlock->m_uiSize);
	SKMAC_ASSERT( *pEndMask == END_MASK);
...
void SKDebugMem::InsertBlock (Block* pBlock)
{
...
void SKDebugMem::RemoveBlock (Block* pBlock)
{
	if (pBlock->m_pPrev)
	{
...
void SKDebugMem::PrintInfo()
{


	SKOutPutDebugString(_T("#########################  begin print leak mem  ######################\n"));

	SKOutPutDebugString(_T("Max Byte Num: %d\n"),m_uiMaxNumBytes);

	SKOutPutDebugString(_T("Max Block Num: %d\n"),m_uiMaxNumBlocks);

	SKOutPutDebugString(_T("Totle Size: %d\n"),m_uiNumBytes);

	SKOutPutDebugString(_T("Block Num: %d\n"),m_uiNumBlocks);


	SKOutPutDebugString(_T("New Call: %d\n"),m_uiNumNewCalls);

	SKOutPutDebugString(_T("Delete Call: %d\n"),m_uiNumDeleteCalls);

	if (m_pHead)
	{
		SKOutPutDebugString(_T("Memory Leak:\n"));
	}
	else
	{
		SKOutPutDebugString(_T("No Memory Leak\n"));
	}
	Block * pBlock = m_pHead;
	...
	while (pBlock)
	{
		...
		uiLeakNum++;
		SKOutPutDebugString(_T("$$$$$$$$$$$$$$$$  Leak %d  $$$$$$$$$$$$$$$$$\n"),uiLeakNum);
		SKOutPutDebugString(_T("Size: %d\n"),pBlock->m_uiSize);
		SKOutPutDebugString(_T("Is Array:%d\n"),pBlock->m_bIsArray);
		...
		for (unsigned int i = 0 ; i < pBlock->m_uiStackInfoNum ; i++)
		{
			...
			SKOutPutDebugString(_T("%s(%d)\n"),szFile,line);
		
		}
		SKOutPutDebugString(_T("$$$$$$$$$$$$$$$$$ Leak %d  $$$$$$$$$$$$$$$$$$$\n"),uiLeakNum);
		pBlock = pBlock->m_pNext;
	}
	SKOutPutDebugString(_T("leak block total num : %d\n"),uiLeakNum);

	SKOutPutDebugString(_T("#########################  end print leak mem  ######################\n"));
}
SKMemWin32::SKMemWin32()
{
	...
	SKMAC_ASSERT(!(PageSize&(PageSize-1)));
	...
	SKMAC_ASSERT(POOL_MAX-1==PoolTable[POOL_COUNT-1].BlockSize);
}
SKMemWin32::~SKMemWin32()
{
...
SKMemWin32::FPoolInfo* SKMemWin32::FFreeMem::GetPool()
{
	return (FPoolInfo*)((INT)this & 0xffff0000);
}
void* SKMemWin32::Allocate (unsigned int uiSize,unsigned int uiAlignment,bool bIsArray)
{
	ms_MemLock.Lock();
	FFreeMem* Free;
	if( uiSize<POOL_MAX )
	{
		...
		SKMAC_ASSERT(uiSize<=Table->BlockSize);
		...
		if( !Pool )
		{
			...
			SKMAC_ASSERT(Blocks>=1);
			SKMAC_ASSERT(Blocks*Table->BlockSize<=Bytes);
		...
		Pool->Taken++;
		SKMAC_ASSERT(Pool->FirstMem);
		SKMAC_ASSERT(Pool->FirstMem->Blocks>0);
	...
	else
	{
		...
		SKMAC_ASSERT(!((SIZE_T)Free&65535));
...
void SKMemWin32::Deallocate (char* pcAddr, unsigned int uiAlignment,bool bIsArray)
{
	ms_MemLock.Lock();
	if( !pcAddr )
	{
...
void* SKCMem::Allocate (unsigned int uiSize,unsigned int uiAlignment,bool bIsArray)
{
	if (!uiSize)
	{
		return NULL;
...
void SKCMem::Deallocate (char* pcAddr, unsigned int uiAlignment,bool bIsArray)
{
	if (!pcAddr)
	{
		return;
...
SKStackMem::SKStackMem(unsigned int uiDefaultChunkSize)
{
	Top = NULL;				
	End = NULL;				
...
SKStackMem::~SKStackMem()
{
	...
	while( UnusedChunks )
	{
		...
		SKMemObject::GetMemManager().Deallocate((char *)Old,0,true);
	}
}

BYTE* SKStackMem::AllocateNewChunk( INT MinSize )
{
	...
	if( !Chunk )
	{
		...
		Chunk           = (FTaggedMemory*)SKMemObject::GetMemManager().Allocate( DataSize + sizeof(FTaggedMemory),0,true);
...
void SKStackMem::FreeChunks( FTaggedMemory* NewTopChunk )
{
...
void* SKStackMem::Allocate (unsigned int uiSize,unsigned int uiAlignment,bool bIsArray)
{
	// Debug checks.
	SKMAC_ASSERT(uiSize>=0);
	if (uiAlignment > 0)
	{
		SKMAC_ASSERT((uiAlignment&(uiAlignment-1))==0);
	}
	SKMAC_ASSERT(Top<=End);
	SKMAC_ASSERT(NumMarks > 0);
...
void SKStackMem::Clear()
{
	FreeChunks(NULL);
}
SKMemManager& SKMemObject::GetMemManager()
{	
#ifdef _DEBUG
	static SKDebugMem g_MemManager;
#else
	static SKMemWin32 g_MemManager;
#endif 

	return g_MemManager;
}
SKStackMem& SKMemObject::GetStackMemManager ()
{
	static SKStackMem g_StackMemManager;
	return g_StackMemManager;
}
SKMemObject::SKMemObject()
{
